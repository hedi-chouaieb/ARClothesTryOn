...
namespace ARClothesTryOn
{
    public class PoseTrackingSolution : ImageSourceSolution<PoseTrackingGraph>
    {
        ...
        [SerializeField] bool activatePoseDetectionAnnotationController;
        [SerializeField] bool activatePoseLandmarksAnnotationController;
        [SerializeField] bool activatePoseWorldLandmarksAnnotationController;
        [SerializeField] bool activateSegmentationMaskAnnotationController;
        [SerializeField] bool activateRoiFromLandmarksAnnotationController;
        ...
        protected override IEnumerator WaitForNextValue()
        {
            ...
            if (activatePoseDetectionAnnotationController)
                _poseDetectionAnnotationController.DrawNow(poseDetection);
            if (activatePoseLandmarksAnnotationController)
                _poseLandmarksAnnotationController.DrawNow(poseLandmarks);
            if (activatePoseWorldLandmarksAnnotationController)
                _poseWorldLandmarksAnnotationController.DrawNow(poseWorldLandmarks);
            if (activateSegmentationMaskAnnotationController)
                _segmentationMaskAnnotationController.DrawNow(segmentationMask);
            if (activateRoiFromLandmarksAnnotationController)
                _roiFromLandmarksAnnotationController.DrawNow(roiFromLandmarks);

            bodyPoseTracker.UpdateClothes(poseWorldLandmarks?.Landmark, poseLandmarks?.Landmark);
            // mediapipe2UnitySkeletonController?.Refresh(poseWorldLandmarks);
        }
